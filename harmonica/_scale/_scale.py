...
class ScaleFunc:
    ...
    def compose(self, other: ScaleFunc) -> ScaleFunc:
        ...
        cardinality = self.cardinality * other.cardinality // math.gcd(self.cardinality, other.modulus)
        pattern = [
            chroma - transposition for chroma in self.eval(other.eval(range(1, cardinality + 1)))
        ]
    ...
    def _index(self, pitch: int) -> int:
        ...
        return self._rmap.index(r) + (((pitch - self.transposition) // self.modulus) * self.cardinality)
